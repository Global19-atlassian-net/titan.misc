...
class DirectoryTraversalEdit(QWidget):
  def __init__(self, parent = None):
    super(DirectoryTraversalEdit, self).__init__(parent)
    
    self.tcType = "DirectoryTraversal"
    ...
    self.depthSpacer = QSpacerItem(0, 0, QSizePolicy.Expanding, QSizePolicy.Minimum)
    
    self.depthLayout.addWidget(self.depthLabel)
    self.depthLayout.addWidget(self.depthEdit)
    self.depthLayout.addSpacerItem(self.depthSpacer)
    ...
    self.vLayout.addLayout(self.depthLayout)
  ...
  def newDepth(self, value):
    self.depthEdit.setValue(value)
  ...
  def serialize(self):
    offset = "      "
    s = ""
    s = s + offset + "DirectoryTraversal :=\n"
    s = s + offset + "{\n"
    if len(self.widgets) != 0:
      s = s + offset + "  " + "{\n"
      if self.widgets[0].packetType == "request":
        s = s + offset + "    depth := " + str(self.depthEdit.value()) + ",\n"
      ...
      for i in range(1, len(self.widgets)):
        if self.widgets[i].packetType != "response":
          s = s + ",\n" + offset + "  " + "{\n"
          if self.widgets[i].packetType == "request":
            s = s + offset + "    depth := " + str(self.depthEdit.value()) + ",\n"
